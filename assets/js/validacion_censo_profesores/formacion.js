$(function () {
    if($('.btn_agregar_ejercicio').length){
        $('.btn_agregar_ejercicio').on('click', function() {
            ...
            $.ajax({
                url: site_url + '/validacion_censo_profesores/formacion_ejercicio_profesional',
                method: 'POST',
                dataType: "json",
    ...
    if($('.btn_agregar_formacion_docente_modal').length){
        $('.btn_agregar_formacion_docente_modal').on('click', function() {
            data_ajax(site_url+'/validacion_censo_profesores/formacion_docente_formulario/', null, '#modal_content');
        });
    }
    if($('.btn_editar_fi').length){
        $('.btn_editar_fi').on('click', function() {
            data_ajax(site_url+'/validacion_censo_profesores/formacion_salud_formulario/'+$(this).attr('data-value'), null, '#modal_content');
        });
    }
    if($('.btn_eliminar_fi').length){
        $('.btn_eliminar_fi').on('click', function() {
            ...
            apprise(confirmar_eliminacion, {verify: true}, function(btnClick) {
                if (btnClick) {
                    $.ajax({
                        url: site_url + '/validacion_censo_profesores/eliminar_formacion_salud/'+data_value,
                        method: 'POST',
                        dataType: "json",
                        beforeSend: function(xhr) {
                            $('#mensaje1').html(create_loader());
    ...
    if($('.btn_editar_fd').length){
        $('.btn_editar_fd').on('click', function() {
            data_ajax(site_url+'/validacion_censo_profesores/formacion_docente_formulario/'+$(this).attr('data-value'), null, '#modal_content');
        });
    }
    if($('.btn_eliminar_fd').length){
        $('.btn_eliminar_fd').on('click', function() {
            ...
            apprise(confirmar_eliminacion, {verify: true}, function(btnClick) {
                if (btnClick) {
                    $.ajax({
                        url: site_url + '/validacion_censo_profesores/eliminar_formacion_docente/'+data_value,
                        method: 'POST',
                        dataType: "json",
                        beforeSend: function(xhr) {
