...
$(function() {
    ...
    if($('#tipo_curso').length){
        $( "#tipo_curso" ).change(function() {
            if($(this).val()!=""){
                data_ajax(site_url+'/perfil/curso/'+$(this).val(), null, '#capa_curso');
            }
        });
        <?php
        if(isset($dir_tes['TIP_CURSO_CVE']) && !empty($dir_tes['TIP_CURSO_CVE'])){ ?>
            data_ajax(site_url+"/perfil/curso/"+$('#tipo_curso').val()+"/<?php echo ((isset($dir_tes['CURSO_CVE']) && !empty($dir_tes['CURSO_CVE'])) ? $dir_tes['CURSO_CVE'] : '')?>", null, '#capa_curso');
        <?php } ?>
    }
