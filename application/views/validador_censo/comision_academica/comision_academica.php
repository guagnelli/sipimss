			                                ...
			                                <table class="table table-striped table-hover table-bordered" id="tabla_comision_academica">
			                                    ...
			                                    <tbody>
		                                    		...
		                                    		foreach ($comisiones[$key_tc] as $key_ca => $comision_academica) {
                                                        ...
                                                        $valido_eliminar_editar = $this->seguridad->verificar_liga_validar($comision_academica['IS_VALIDO_PROFESIONALIZACION'], $comision_academica['IS_CARGA_SISTEMA']);
                                                        $btn_validar = ($valido_eliminar_editar) ? '<button type="button" class="btn btn-link btn-sm btn_validar_ca" aria-expanded="false" data-toggle="modal" data-target="#modal_censo" data-value="'.$id.'" data-com="'.$this->seguridad->encrypt_base64($key_tc).'" onclick="validar_ca(this);" data-valid="'.$this->seguridad->encrypt_base64($this->config->item('ACCION_GENERAL')['VALIDAR']['valor']).'">'.$string_values['validar'].'</button>' : '';
                                                        ///////////Fin ver liga de validación
														echo '<tr id="tr_'.$id.'">
															<td class="text-center">'.$this->seguridad->html_verificar_evaluado_issistema_valido($comision_academica['IS_VALIDO_PROFESIONALIZACION'], $comision_academica['IS_CARGA_SISTEMA'], $comision_academica['validation_estado']).'</td>';
