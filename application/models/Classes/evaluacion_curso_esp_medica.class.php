...
Class evaluacion_curso_esp_medica {
	...
	public function evaluacion_curso_esp_medica(){
     ...
     *
     */
	public function New_evaluacion_curso_esp_medica($EVA_CUR_VALIDO,$EVA_CUR_PUNTOS_CURSO,$TABU_ACT_DOCENTE_CVE,$ROL_EVALUADOR_CVE,$EVA_CUR_CATEGORIA,$EVA_CUR_MSG_RE_EVALUACION,$EVA_CUR_PUNTOS_CURSO_ORIGINAL,$EMP_ESP_MEDICA_CVE,$FCH_EVALUACION_CURSO_GAECUD){
	...
	public function Load_from_key($key_row){
		$result = $this->connection->RunQuery("Select * from evaluacion_curso_esp_medica where EVA_CURSO_CVE = \"$key_row\" ");
	...
	public function Delete_row_from_key($key_row){
		$this->connection->RunQuery("DELETE FROM evaluacion_curso_esp_medica WHERE EVA_CURSO_CVE = $key_row");
	}
	...
	public function Save_Active_Row(){
		$this->connection->RunQuery("UPDATE evaluacion_curso_esp_medica set EVA_CUR_VALIDO = \"$this->EVA_CUR_VALIDO\", EVA_CUR_PUNTOS_CURSO = \"$this->EVA_CUR_PUNTOS_CURSO\", TABU_ACT_DOCENTE_CVE = \"$this->TABU_ACT_DOCENTE_CVE\", ROL_EVALUADOR_CVE = \"$this->ROL_EVALUADOR_CVE\", EVA_CUR_CATEGORIA = \"$this->EVA_CUR_CATEGORIA\", EVA_CUR_MSG_RE_EVALUACION = \"$this->EVA_CUR_MSG_RE_EVALUACION\", EVA_CUR_PUNTOS_CURSO_ORIGINAL = \"$this->EVA_CUR_PUNTOS_CURSO_ORIGINAL\", EMP_ESP_MEDICA_CVE = \"$this->EMP_ESP_MEDICA_CVE\", FCH_EVALUACION_CURSO_GAECUD = \"$this->FCH_EVALUACION_CURSO_GAECUD\" where EVA_CURSO_CVE = \"$this->EVA_CURSO_CVE\"");
	}
	...
	public function Save_Active_Row_as_New(){
		$this->connection->RunQuery("Insert into evaluacion_curso_esp_medica (EVA_CUR_VALIDO, EVA_CUR_PUNTOS_CURSO, TABU_ACT_DOCENTE_CVE, ROL_EVALUADOR_CVE, EVA_CUR_CATEGORIA, EVA_CUR_MSG_RE_EVALUACION, EVA_CUR_PUNTOS_CURSO_ORIGINAL, EMP_ESP_MEDICA_CVE, FCH_EVALUACION_CURSO_GAECUD) values (\"$this->EVA_CUR_VALIDO\", \"$this->EVA_CUR_PUNTOS_CURSO\", \"$this->TABU_ACT_DOCENTE_CVE\", \"$this->ROL_EVALUADOR_CVE\", \"$this->EVA_CUR_CATEGORIA\", \"$this->EVA_CUR_MSG_RE_EVALUACION\", \"$this->EVA_CUR_PUNTOS_CURSO_ORIGINAL\", \"$this->EMP_ESP_MEDICA_CVE\", \"$this->FCH_EVALUACION_CURSO_GAECUD\")");
	}
	...
	public function GetKeysOrderBy($column, $order){
		$keys = array(); $i = 0;
		$result = $this->connection->RunQuery("SELECT EVA_CURSO_CVE from evaluacion_curso_esp_medica order by $column $order");
			while($row = $result->fetch_array(MYSQLI_ASSOC)){
     ...
     */
	public function endevaluacion_curso_esp_medica(){
