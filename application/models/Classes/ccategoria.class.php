...
Class ccategoria {
	...
	private $cve_tipo_categoria; //int(11)
     ...
     *
     */
	public function New_ccategoria($id_cat,$nom_categoria,$cve_tipo_categoria,$nom_tipo_cat){
		$this->id_cat = $id_cat;
	...
	public function Load_from_key($key_row){
		$result = $this->connection->RunQuery("Select * from ccategoria where des_clave = \"$key_row\" ");
	...
	public function Delete_row_from_key($key_row){
		$this->connection->RunQuery("DELETE FROM ccategoria WHERE des_clave = $key_row");
	}
	...
	public function Save_Active_Row(){
		$this->connection->RunQuery("UPDATE ccategoria set id_cat = \"$this->id_cat\", nom_categoria = \"$this->nom_categoria\", cve_tipo_categoria = \"$this->cve_tipo_categoria\", nom_tipo_cat = \"$this->nom_tipo_cat\" where des_clave = \"$this->des_clave\"");
	}
	...
	public function Save_Active_Row_as_New(){
		$this->connection->RunQuery("Insert into ccategoria (id_cat, nom_categoria, cve_tipo_categoria, nom_tipo_cat) values (\"$this->id_cat\", \"$this->nom_categoria\", \"$this->cve_tipo_categoria\", \"$this->nom_tipo_cat\")");
	}
	...
	public function GetKeysOrderBy($column, $order){
		$keys = array(); $i = 0;
		$result = $this->connection->RunQuery("SELECT des_clave from ccategoria order by $column $order");
			while($row = $result->fetch_array(MYSQLI_ASSOC)){
	...
	/**
	 * @return cve_tipo_categoria - int(11)
	 */
