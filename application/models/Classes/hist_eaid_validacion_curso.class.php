...
Class hist_eaid_validacion_curso {
	...
	public function hist_eaid_validacion_curso(){
     ...
     *
     */
	public function New_hist_eaid_validacion_curso($VALIDACION_CVE,$VAL_CUR_EST_CVE,$VAL_CUR_COMENTARIO,$VAL_CUR_FCH,$EAID_CVE){
	...
	public function Load_from_key($key_row){
		$result = $this->connection->RunQuery("Select * from hist_eaid_validacion_curso where HIST_VAL_CURSO_CVE = \"$key_row\" ");
	...
	public function Delete_row_from_key($key_row){
		$this->connection->RunQuery("DELETE FROM hist_eaid_validacion_curso WHERE HIST_VAL_CURSO_CVE = $key_row");
	}
	...
	public function Save_Active_Row(){
		$this->connection->RunQuery("UPDATE hist_eaid_validacion_curso set VALIDACION_CVE = \"$this->VALIDACION_CVE\", VAL_CUR_EST_CVE = \"$this->VAL_CUR_EST_CVE\", VAL_CUR_COMENTARIO = \"$this->VAL_CUR_COMENTARIO\", VAL_CUR_FCH = \"$this->VAL_CUR_FCH\", EAID_CVE = \"$this->EAID_CVE\" where HIST_VAL_CURSO_CVE = \"$this->HIST_VAL_CURSO_CVE\"");
	}
	...
	public function Save_Active_Row_as_New(){
		$this->connection->RunQuery("Insert into hist_eaid_validacion_curso (VALIDACION_CVE, VAL_CUR_EST_CVE, VAL_CUR_COMENTARIO, VAL_CUR_FCH, EAID_CVE) values (\"$this->VALIDACION_CVE\", \"$this->VAL_CUR_EST_CVE\", \"$this->VAL_CUR_COMENTARIO\", \"$this->VAL_CUR_FCH\", \"$this->EAID_CVE\")");
	}
	...
	public function GetKeysOrderBy($column, $order){
		$keys = array(); $i = 0;
		$result = $this->connection->RunQuery("SELECT HIST_VAL_CURSO_CVE from hist_eaid_validacion_curso order by $column $order");
			while($row = $result->fetch_array(MYSQLI_ASSOC)){
     ...
     */
	public function endhist_eaid_validacion_curso(){
