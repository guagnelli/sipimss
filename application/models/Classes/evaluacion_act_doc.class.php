...
Class evaluacion_act_doc {
	...
	public function evaluacion_act_doc(){
     ...
     *
     */
	public function New_evaluacion_act_doc($TOTAL_PUNTOS,$MSG_INCONFORMIDAD,$EVA_CURSO_CVE,$EST_EVALUACION_CVE){
	...
	public function Load_from_key($key_row){
		$result = $this->connection->RunQuery("Select * from evaluacion_act_doc where TOTAL_PUNTOS = \"$key_row\" ");
	...
	public function Delete_row_from_key($key_row){
		$this->connection->RunQuery("DELETE FROM evaluacion_act_doc WHERE TOTAL_PUNTOS = $key_row");
	}
	...
	public function Save_Active_Row(){
		$this->connection->RunQuery("UPDATE evaluacion_act_doc set TOTAL_PUNTOS = \"$this->TOTAL_PUNTOS\", MSG_INCONFORMIDAD = \"$this->MSG_INCONFORMIDAD\", EVA_CURSO_CVE = \"$this->EVA_CURSO_CVE\", EST_EVALUACION_CVE = \"$this->EST_EVALUACION_CVE\" where TOTAL_PUNTOS = \"$this->TOTAL_PUNTOS\"");
	}
	...
	public function Save_Active_Row_as_New(){
		$this->connection->RunQuery("Insert into evaluacion_act_doc (TOTAL_PUNTOS, MSG_INCONFORMIDAD, EVA_CURSO_CVE, EST_EVALUACION_CVE) values (\"$this->TOTAL_PUNTOS\", \"$this->MSG_INCONFORMIDAD\", \"$this->EVA_CURSO_CVE\", \"$this->EST_EVALUACION_CVE\")");
	}
	...
	public function GetKeysOrderBy($column, $order){
		$keys = array(); $i = 0;
		$result = $this->connection->RunQuery("SELECT TOTAL_PUNTOS from evaluacion_act_doc order by $column $order");
			while($row = $result->fetch_array(MYSQLI_ASSOC)){
     ...
     */
	public function endevaluacion_act_doc(){
